...
passport.use('local-signup', new ppLocalStrategy({
    ...
    .then(function (user) {
        ...
        } else if (usernLength < 3) {
            console.log(">>username too short: " + usernLength);
            req.session.error = "Username needs to be longer than 3 characters";
            return done(null, false);
        } else if (passLength < 6) {
            console.log(">>password too short: " + passLength);
...
var forum = require('./forumFiles/forum-routes.js')(app, io);
...
io.on('connection', function (socket){
    ...
    socket.on('requestPosts', function (query){
        console.log("f# Requesting post of " + query);
        try{
            styx.getPosts(query)
            .then(function (posts){
                console.log("ubsafv" + posts);
